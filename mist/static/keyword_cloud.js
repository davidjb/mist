...
var jcu_colours_indices = [];
for (var i = 0; i <= jcu_colours['blue'].length; i++) {
    jcu_colours_indices.push(i);
}

function word_cloud(selector) {
    var target = d3.select(selector);
    var palette = jcu_colours[target.attr('data-colour') || 'green'];
    var fill = d3.scale.ordinal().domain(jcu_colours_indices).range(palette);
